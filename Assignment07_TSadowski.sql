...
-- 2021-03-01,Tara Sadowski,Created File
-- 2021-03-02,Tara Sadowski,Entered Function Codes 1-6
-- 2021-03-02,Tara Sadowski,Entered Function Codes 7-8
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_TSadowski')
	 Begin 
	  Alter Database [Assignment07DB_TSadowski] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_TSadowski;
	 End
	Create Database Assignment07DB_TSadowski;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_TSadowski;
...
Order by ProductName, I.InventoryDate, InventoryCount;  -- Saw that doing alias as same name as shown on example, I need to sort by the view's date or will sort alphabetically.
go
...
As
  ...
  ,Format(InventoryDate,'MMMM, yyyy') as InventoryDate
  ,[Count] as InventoryCount
  ,IIF(Month(InventoryDate)=1,0,(IsNull(Lag([Count]) over(Order by ProductName,Month(InventoryDate)),0))) as PreviousMonthCount
  From vInventories as I
  Inner Join vProducts as P
  On I.ProductID=P.ProductID
 Order by ProductName, I.InventoryDate, InventoryCount;
...
As
 ...
 Order by ProductName, DATEPART(mm,CAST(InventoryDate as DATETIME)), InventoryCount; --Previous solution caused issues building out of saved view, but was able to find a function to solve the issue
go
 ...
 Returns Table
  As
   Return(
	 ...
	 Where dbo.vProductInventoriesWithPreviousMonthCountsWithKPIs.CountvsPreviousCountKPI = @CountvsPreviousCountKPI
